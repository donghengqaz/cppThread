...
void te3(estd::time_t t)
{
    try {
        ...
        class socket s5(SOCK_STREAM, 5055, true);

        class iolist rl;
        class iolist wl; // just not use
        class iolist el; // just not use

        std::cout << "connect " << s4.connect(estd::ipaddr("www.baidu.com", 443)) << std::endl;
...
static void main_test(void)
{
    ...
    class thread t1(5, 8192, te1, 500, a1);
    class thread t2(5, 8192, te2, 15, a2);
    class thread t3(5, 8192, te3, 5.1);
